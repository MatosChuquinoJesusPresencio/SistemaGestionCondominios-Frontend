...
const FloorTable = ({ data, torres, onEdit, onDelete }) => {
    ...
    const filtered = useMemo(() => {
        return data.filter(f => {
            const torre = torres.find(t => t.id === f.id_torre);
    ...
    }, [data, searchTerm, towerFilter, torres]);
        ...
        <>
            ...
            <div className="table-responsive">
                <Table hover className="align-middle mb-0 custom-table">
                    ...
                    <tbody>
                        {paginated.length > 0 ? paginated.map(floor => (
                            <tr key={floor.id} className="border-bottom border-light">
                                <td className="px-4 py-3">
                                    <div className="d-flex align-items-center gap-3">
                                        ...
                                        <div className="fw-bold text-dark">Piso {floor.numero_piso}</div>
                                    </div>
                                </td>
                                <td className="py-3">
                                ...
                                <td className="px-4 py-3 text-end">
                                    <div className="d-flex justify-content-end gap-2">
                                        <Button variant="light" size="sm" className="text-warning" onClick={() => onEdit(floor)}>
                                            <FaEdit />
                                        </Button>
                                        <Button variant="light" size="sm" className="text-danger" onClick={() => onDelete(floor)}>
                                            <FaTrash />
                                        </Button>
                                    </div>
                                </td>
                            ...
                            <EmptyState colSpan={3} message="No hay pisos registrados." icon={FaLayerGroup} />
                        )}
                    </tbody>
                </Table>
            </div>
