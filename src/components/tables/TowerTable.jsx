...
const TowerTable = ({ data, onEdit, onDelete }) => {
    ...
    const filtered = useMemo(() => {
        return data.filter(t => t.nombre.toLowerCase().includes(searchTerm.toLowerCase()));
    }, [data, searchTerm]);
        ...
        <>
            ...
            <div className="table-responsive">
                <Table hover className="align-middle mb-0 custom-table">
                    ...
                    <tbody>
                        {paginated.length > 0 ? paginated.map(tower => (
                            <tr key={tower.id} className="border-bottom border-light">
                                ...
                                <td className="px-4 py-3 text-end">
                                    <div className="d-flex justify-content-end gap-2">
                                        <Button variant="light" size="sm" className="text-warning" onClick={() => onEdit(tower)}>
                                            <FaEdit />
                                        </Button>
                                        <Button variant="light" size="sm" className="text-danger" onClick={() => onDelete(tower)}>
                                            <FaTrash />
                                        </Button>
                                    </div>
                                </td>
                            ...
                            <EmptyState colSpan={3} message="No hay torres registradas." icon={FaBuilding} />
                        )}
                    </tbody>
                </Table>
            </div>
