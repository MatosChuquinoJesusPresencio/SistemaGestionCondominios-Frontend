...
const AptoTable = ({ data, pisos, torres, usuarios, onEdit, onDelete }) => {
    ...
    const filtered = useMemo(() => {
        return data.filter(a => {
            ...
            const matchesTower = towerFilter === "all" || piso?.id_torre.toString() === towerFilter;
        ...
        <>
            <div className="mb-4">
                <SearchBar 
                    ...
                    onFilterChange={(val) => { setTowerFilter(val); setCurrentPage(1); }}
                    filterOptions={[
            ...
            <div className="table-responsive">
                <Table hover className="align-middle mb-0 custom-table">
                    ...
                    <tbody>
                        {paginated.length > 0 ? paginated.map(apto => {
                                ...
                                <tr key={apto.id} className="border-bottom border-light">
                                    ...
                                    <td className="py-3">
                                        <div className="d-flex align-items-center gap-2 small">
                                            ...
                                            <span className={owner ? "text-dark fw-semibold" : "text-danger italic"}>
                                    ...
                                    <td className="px-4 py-3 text-end">
                                        <div className="d-flex justify-content-end gap-2">
                                            <Button variant="light" size="sm" className="text-warning" onClick={() => onEdit(apto)}>
                                                <FaEdit />
                                            </Button>
                                            <Button variant="light" size="sm" className="text-danger" onClick={() => onDelete(apto)}>
                                                <FaTrash />
                                            </Button>
                                        </div>
                                    </td>
                            ...
                            <EmptyState colSpan={4} message="No hay apartamentos registrados." icon={FaHome} />
                        )}
                    </tbody>
                </Table>
            </div>
