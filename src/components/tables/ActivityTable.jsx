...
import { FaShoppingCart, FaCar, FaClock, FaCalendarAlt, FaUser, FaHome, FaBuilding } from 'react-icons/fa';
                ...
                <Table hover className="align-middle mb-0 custom-table">
                    <thead className="bg-light text-muted small text-uppercase">
                        <tr>
                            ...
                            <th className={showCondo ? "py-3 border-0" : "px-4 py-3 border-0"}>Unidad</th>
                    ...
                    <tbody>
                        {data.length > 0 ? data.map(log => (
                            <tr key={log.id} className="border-bottom border-light">
                                {showCondo && (
                                    <td className="px-4 py-3">
                                        <div className="fw-bold text-dark small">{log.condoNombre || "N/A"}</div>
                                    </td>
                                )}
                                <td className={showCondo ? "py-3" : "px-4 py-3"}>
                                    <div className="small"><FaHome className="me-1 text-muted" /> {log.aptoNumero || log.id_apartamento}</div>
                                </td>
                                <td className="py-3">
                                    <div className="small fw-medium">{log.carritoNombre || `Carrito ${log.id_carrito}`}</div>
                                </td>
                                <td className="py-3">
                                    <div className="small text-muted"><FaUser className="me-1" /> {log.usuarioNombre || log.solicitante}</div>
                                </td>
                                <td className="py-3">
                            ...
                            <EmptyState colSpan={showCondo ? 6 : 5} message="No hay registros de carritos." icon={FaShoppingCart} />
                        )}
                    </tbody>
                </Table>
            </div>
        ...
        <div className="table-responsive">
            <Table hover className="align-middle mb-0 custom-table">
                <thead className="bg-light text-muted small text-uppercase">
                    <tr>
                        {showCondo && <th className="px-4 py-3 border-0">Condominio</th>}
                        <th className={showCondo ? "py-3 border-0" : "px-4 py-3 border-0"}>Vehículo / Placa</th>
                    ...
                    {data.length > 0 ? data.map(log => (
                        <tr key={log.id} className="border-bottom border-light">
                            ...
                            <td className="py-3 text-center">
                                <Badge bg="dark" className="rounded-2">{log.estacionamientoNumero || log.id_estacionamiento}</Badge>
                            </td>
                            <td className="py-3">
                                <div className="x-small"><FaCalendarAlt className="me-1 text-muted" /> {formatDateTime(log.fecha_entrada)}</div>
                            </td>
                            <td className="py-3">
                                <div className="x-small">{log.fecha_salida ? formatDateTime(log.fecha_salida) : "---"}</div>
                            </td>
                            <td className="px-4 py-3 text-center">
                        ...
                        <EmptyState colSpan={showCondo ? 6 : 5} message="No hay registros de acceso vehicular." icon={FaCar} />
                    )}
                </tbody>
            </Table>
        </div>
