...
const ACApartamentosPage = () => {
    ...
    const aptosCondo = useMemo(() => {
        ...
        const pisosIds = pisos.filter(p => torresIds.includes(p.id_torre)).map(p => p.id);
        
        return apartamentos.filter(a => pisosIds.includes(a.id_piso)).map(a => {
    ...
    const filteredAptos = useMemo(() => {
        return aptosCondo.filter(apto => {
            ...
            const matchesTower = towerFilter === "all" || apto.torreNombre === towerFilter;
    ...
    const handleAddResident = (data) => {
        const newId = inquilinosTemporales.length > 0 ? Math.max(...inquilinosTemporales.map(i => i.id)) + 1 : 1;
        const newInquilino = {
            id: newId,
        ...
        updateTable('inquilinos_temporales', [...inquilinosTemporales, newInquilino]);
        reset();
    };
        ...
        <AnimatedPage>
            <div className="container-fluid py-4 bg-light min-vh-100">
                ...
                <Row className="g-4 mb-5">
                    <StatCard icon={FaBuilding} label="Total Unidades" value={stats.total} colorClass="primary" />
                    <StatCard icon={FaCheckCircle} label="Ocupados" value={stats.ocupados} colorClass="success" />
                    <StatCard icon={FaInfoCircle} label="Sin Propietario" value={stats.sinPropietario} colorClass="warning" />
                    <StatCard icon={FaUsers} label="Población Estimada" value={stats.totalResidentes} colorClass="info" />
                </Row>
                                        ...
                                        <tr key={apto.id} className="border-bottom border-light">
                                            <td className="px-4 py-3">
                                                <div className="d-flex align-items-center gap-3">
                                                    <div className="apto-badge">
                                                        {apto.numero}
                                                    </div>
                                                    <div>
                                                        <div className="fw-bold text-dark">Apto {apto.numero}</div>
                                                        <div className="x-small text-muted">{apto.metraje} m²</div>
                                                    </div>
                                                </div>
                                            </td>
                                            <td className="py-3">
                                            ...
                                            <td className="py-3">
                                                <div className="d-flex align-items-center gap-2">
                                                    ...
                                                    <span className={`small ${apto.id_usuario ? "text-dark fw-semibold" : "text-danger italic"}`}>
                                                        {apto.ownerName}
                                                    </span>
                                                </div>
                                            </td>
                                                ...
                                                <div className="d-flex flex-wrap gap-1">
                                                    {apto.residents.length > 0 ? (
                                                        apto.residents.map(r => (
                                                            <Badge key={r.id} bg="light" className="text-primary border border-primary border-opacity-10 fw-normal">
                                                                {r.nombre.split(' ')[0]}
                                                            </Badge>
                                                        ))
                                                    ) : (
            ...
            <Modal show={showResidentModal} onHide={() => setShowResidentModal(false)} centered size="lg">
                                ...
                                <Table borderless hover size="sm" className="mb-0">
                                    ...
                                    <tbody>
                                        {currentApto.residents.map(r => (
                                            <tr key={r.id} className="align-middle border-bottom border-white">
                                                <td className="py-2 fw-medium">{r.nombre}</td>
                                                <td className="py-2 text-muted small">{r.dni}</td>
                                                <td className="py-2 text-end">
                                                    <Button variant="link" className="text-danger p-0" onClick={() => handleRemoveResident(r.id)}>
                                                        <FaTrash size={12} />
                                                    </Button>
                                                </td>
                                            </tr>
                    ...
                    <div>
                        ...
                        <Form onSubmit={handleSubmit(handleAddResident)}>
                            ...
                            <div className="d-flex justify-content-end mt-3">
                                <Button type="submit" className="btn-primary-theme rounded-pill px-4 btn-sm">
                ...
                <Modal.Footer className="border-0 p-4 pt-0">
                    <Button variant="light" onClick={() => setShowResidentModal(false)} className="rounded-pill px-4 fw-bold text-secondary">
                        Cerrar
                    </Button>
                </Modal.Footer>
            </Modal>
