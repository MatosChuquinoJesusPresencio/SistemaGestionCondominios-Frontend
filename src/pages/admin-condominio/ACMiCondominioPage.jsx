...
import { FaBuilding, FaMapMarkerAlt, FaCalendarAlt, FaUsersCog, FaCog, FaPlusCircle, FaUsers, FaTools, FaExclamationTriangle, FaInfoCircle, FaEdit, FaSave, FaTimes } from "react-icons/fa";
...
const ACMiCondominioPage = () => {
    ...
    // Buscar el condominio asignado al usuario actual
    const condominio = getTable('condominios').find(c => c.id === authUser?.id_condominio);
    const configuraciones = getTable('configuraciones');
    const config = configuraciones.find(c => c.id_condominio === condominio?.id);

    // Cargar datos en el formulario cuando se abre el modal
    ...
    const onSubmit = (data) => {
        ...
        if (config) {
            ...
            // Crear nueva
            const newId = configuraciones.length > 0 ? Math.max(...configuraciones.map(c => c.id)) + 1 : 1;
            const newConfig = {
                id: newId,
    ...
    const usersInCondo = getTable('usuarios').filter(u => u.id_condominio === condominio.id);
    const carts = getTable('carritos_carga').filter(c => c.id_condominio === condominio.id);
        ...
        <AnimatedPage>
            <div className="container-fluid py-4 bg-light min-vh-100">
                ...
                <div className="row g-4 mb-5">
                    <StatCard icon={FaBuilding} label="Torres / Bloques" value={torres.length} colorClass="primary" />
                    <StatCard icon={FaPlusCircle} label="Total Departamentos" value={aptos.length} colorClass="success" />
                    <StatCard icon={FaUsers} label="Usuarios Registrados" value={usersInCondo.length} colorClass="warning" />
                    <StatCard icon={FaTools} label="Carritos de Carga" value={carts.length} colorClass="info" />
                </div>
                        ...
                        <Card className="border-0 shadow-sm rounded-4 h-100 overflow-hidden bg-white">
                            ...
                            <Card.Body className="p-4">
                                <div className="p-4 rounded-4 bg-light mb-4">
                                    <h3 className="fw-bold text-primary-theme mb-1">{condominio.nombre}</h3>
                                    <p className="text-secondary mb-3 d-flex align-items-center gap-2">
                                        <FaMapMarkerAlt className="text-danger opacity-75" />
                                        {condominio.direccion}, {condominio.ciudad} ({condominio.pais})
                                    </p>
                                    <div className="badge bg-white text-dark border px-3 py-2 rounded-pill shadow-sm small fw-bold">
                                        <FaCalendarAlt className="me-2 text-primary" />
                                        Registrado el {new Date(condominio.fecha_creacion).toLocaleDateString('es-ES', { day: '2-digit', month: 'long', year: 'numeric' })}
                                    </div>
                                </div>

                                <h6 className="text-muted small text-uppercase fw-bold mb-3 ls-1">Detalles de Infraestructura</h6>
                                <div className="list-group list-group-flush bg-transparent">
                                    ...
                                    <div className="list-group-item bg-transparent px-0 border-light d-flex justify-content-between align-items-center">
                                        <span className="text-secondary small">Departamentos Activos</span>
                                        <span className="fw-bold text-dark">{aptos.length}</span>
                                    </div>
                                    <div className="list-group-item bg-transparent px-0 border-light d-flex justify-content-between align-items-center">
                                        <span className="text-secondary small">Puntos de Acceso Vehicular</span>
                                        <span className="fw-bold text-dark">2 (Entrada/Salida)</span>
                                    </div>
                                </div>
                            </Card.Body>
                        </Card>
                            ...
                            <Card.Body className="p-4">
                                {config ? (
                                    <div className="row g-4">
                                        <Col md={6}>
                                            <div className="p-4 rounded-4 border bg-white shadow-sm hover-up transition-all h-100">
                                                <div className="text-muted small text-uppercase fw-bold mb-3">Límites de Vehículos</div>
                                                <div className="row text-center">
                                                    <Col>
                                                        <div className="fs-3 fw-bold text-dark">{config.max_autos}</div>
                                                        <div className="x-small text-muted fw-bold">AUTOS</div>
                                                    </Col>
                                                    <div className="vr p-0 opacity-10"></div>
                                                    <Col>
                                                        <div className="fs-3 fw-bold text-dark">{config.max_motos}</div>
                                                        <div className="x-small text-muted fw-bold">MOTOS</div>
                                                    </Col>
                                                </div>
                                            </div>
                                        </Col>
                                        <Col md={6}>
                                            <div className="p-4 rounded-4 border bg-white shadow-sm hover-up transition-all h-100">
                                                <div className="text-muted small text-uppercase fw-bold mb-3">Servicio de Carritos</div>
                                                <div className="d-flex align-items-baseline gap-2">
                                                    <span className="fs-3 fw-bold text-dark">{config.tiempo_max_prestamo_min}</span>
                                                    <span className="small text-muted fw-bold">minutos máx.</span>
                                                </div>
                                                <div className="mt-2 pt-2 border-top small text-secondary">
                                                    Penalización: <strong>S/ {config.penalizacion_por_minuto.toFixed(2)}</strong> / min.
                                                </div>
                                            </div>
                                        </Col>
                                        <Col xs={12}>
                                            <div className="alert alert-info bg-info bg-opacity-10 border-info border-opacity-25 rounded-4 p-4 d-flex align-items-center gap-3 mb-0">
                                                ...
                                                <div>
                                                    <h6 className="fw-bold text-dark mb-1">Personalización</h6>
                                                    <p className="small text-secondary mb-0">Como administrador, puedes ajustar estos parámetros para que se adapten a las reglas internas de tu condominio.</p>
                                                </div>
                                            </div>
                                        </Col>
                                    </div>
                                    ...
                                    <div className="text-center py-5">
                                        ...
                                        <h5 className="fw-bold text-dark">Pendiente de Configuración</h5>
                                        <p className="text-secondary small px-5">Establece los límites de vehículos y reglas de carritos para comenzar la gestión operativa.</p>
                                        <Button variant="warning" className="rounded-pill px-4 fw-bold shadow-sm mt-2 border-0 text-white" onClick={handleShowEdit}>
            ...
            <Modal show={showEditModal} onHide={() => setShowEditModal(false)} centered className="border-0">
                ...
                <Modal.Body className="py-4">
                    <Form onSubmit={handleSubmit(onSubmit)}>
                        <h6 className="text-muted small text-uppercase fw-bold mb-3 ls-1">Límites de Estacionamiento</h6>
                        <Row>
                            <Col md={6}>
                                <AuthInput 
                                    ...
                                    name="max_autos"
                                    register={register}
                                    validation={{ required: "Requerido", min: { value: 0, message: "Mínimo 0" } }}
                        ...
                        <h6 className="text-muted small text-uppercase fw-bold mb-3 mt-2 ls-1">Reglas de Carritos de Carga</h6>
                        <Row>
                            <Col md={6}>
                                <AuthInput 
                                    ...
                                    register={register}
                                    validation={{ required: "Requerido", min: { value: 1, message: "Mínimo 1" } }}
                        ...
                        <div className="d-flex justify-content-end gap-2 mt-3">
                            <Button variant="light" onClick={() => setShowEditModal(false)} className="rounded-pill px-4 fw-bold text-secondary border-0">
